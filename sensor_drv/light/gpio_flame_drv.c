...
static struct gpio_light *gpio_lights_100ask;//只用一个gpio引脚不需要分配多个内存空间了
...
static DECLARE_WAIT_QUEUE_HEAD(gpio_light_wait); //声明和初始化一个等待队列头。
...
static ssize_t gpio_light_drv_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	/* 中断处理函数会唤醒该内核read进程，中断触发才有数据过来 */
	wait_event_interruptible(gpio_light_wait, g_key);//如果 g_key 为 0，说明中断处理函数未执行，g_key没有赋值，进程就会进入挂起状态，但硬件中断可打断
...
{
	struct gpio_light *gpio_light = dev_id;//从request_irq传递过来的参数，接收它
	int val;
	val = gpiod_get_value(gpio_light->gpiod);//读取 GPIO 引脚的电平状态,gpiod_get_value返回的是物理？逻辑值。
	

	printk("key %d %d\n", gpio_light->gpio, val);//打印引脚号，引脚电平值
	g_key = (gpio_light->gpio << 8) | val;//将这两个值组合为一个32位值，只有低16位有效
	wake_up_interruptible(&gpio_light_wait);//唤醒在等待队列 gpio_light_wait 中的进程。
...
static int gpio_light_probe(struct platform_device *pdev)
{
	...
	gpio_lights_100ask = kzalloc(sizeof(struct gpio_light) * count, GFP_KERNEL);//分配count个引脚描述结构体内存空间
	for (i = 0; i < count; i++)
	{
		/* 获得引脚和flag */
		gpio_lights_100ask[i].gpio = of_get_gpio_flags(node, i, &flag);//返回非0表示成功，负数出错
		 ...
		 */
		//gpio_lights_100ask[i].gpiod = gpio_to_desc(gpio_lights_100ask[i].gpio);//根据引脚号获得引脚描述信息
		// 正确方式（自动处理GPIO_ACTIVE_LOW等标志）
		gpio_lights_100ask[i].gpiod = devm_gpiod_get_index(&pdev->dev, NULL, i, GPIOD_IN);
		
		gpiod_direction_input(gpio_lights_100ask[i].gpiod);//设置了中断内核可能自动配置引脚方向为输入，所以不配置也能正常读取数据，最好显示配置，避免出问题
		gpio_lights_100ask[i].flag = flag & OF_GPIO_ACTIVE_LOW;//结果为1则为OF_GPIO_ACTIVE_LOW，否则OF_GPIO_SINGLE_ENDED
		gpio_lights_100ask[i].irq  = gpio_to_irq(gpio_lights_100ask[i].gpio);//获得中断号
	}

	for (i = 0; i < count; i++)
	{
		/* 第五个参数用来向中断处理函数传参 ，
   		 * name字段，如"100ask_gpio_light"，与register_chrdev()名称相同
		 */
	...
	/* 注册file_operations 	*/
	major = register_chrdev(0, "100ask_gpio_light", &gpio_light_drv);  /* /dev/100ask_gpio_light */
...
static int gpio_light_remove(struct platform_device *pdev)
{
	...
	kfree(gpio_lights_100ask);//释放内核引脚结构体内存空间
    return 0;
}
...
static int __init gpio_light_init(void)
{
    ...
    err = platform_driver_register(&gpio_lights_driver); //注册驱动opr，包括probe，remove等操作
